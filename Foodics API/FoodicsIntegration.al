codeunit 50131 "Foodics Integration Mgr"
{
                        ...
                        if OneTok.AsObject().Get('total_price', ValTok) and ValTok.IsValue()
                          and OneTok.AsObject().Get('subtotal_price', SubTok) and SubTok.IsValue() then
                            if OneTok.AsObject().Get('tax_exclusive_discount_amount', VATTok) and VATTok.IsValue() and (VATTok.AsValue().AsDecimal() = 0) then
                                ...
                                VatTotal += Sign * ValTok.AsValue().AsDecimal() - (SubTok.AsValue().AsDecimal() - VATTok.AsValue().AsDecimal());
    ...
    var
        ...
        PurchInvNum: Dictionary of [Text, code[35]];
    ...
    begin
                ...
                foreach OneTok in Arr do begin
                    ...
                    if OneTok.AsObject().Get('supplier', vendorTok) then begin
                        ...
                        if not VendorRec.Get(VendorCode) then begin
                            ...
                            if vendorTok.AsObject().Get('name', ValTok) and ValTok.IsValue() and (not ValTok.AsValue().IsNull) then
                                VendorRec.Validate(Name, ValTok.AsValue().AsText());

                            if vendorTok.AsObject().Get('phone', ValTok) and ValTok.IsValue() and (not ValTok.AsValue().IsNull) then
                                VendorRec."Phone No." := ValTok.AsValue().AsText();
                    ...
                    InvoiceNumber := '';
                    if OneTok.AsObject().Get('invoice_number', ValTok) and ValTok.IsValue() then
                        if not ValTok.AsValue().IsNull then
                            InvoiceNumber := ValTok.AsValue().AsText();
                    ...
                    if not PurchVend.ContainsKey("Purchase No.") then begin
                        ...
                        PurchInvNum.Add("Purchase No.", InvoiceNumber);
                    end;
        ...
        KeyList := PurchCost.Keys;
        foreach "Purchase No." in KeyList do begin
            PurchCost.Get("Purchase No.", NetTotal);
            PurchBrID.Get("Purchase No.", BranchID);
            PurchVend.Get("Purchase No.", VendorCode);
            PurchInvNum.Get("Purchase No.", InvoiceNumber);
            ...
            if NetTotal <> 0 then begin
                ...
                PurcInvHeader.Validate("Vendor Invoice No.", InvoiceNumber);
    ...
    begin
        ...
        foreach "Reference No." in PurchCost.Keys do begin
            ...
            // Bring More that 0 and Less than 0 and ignore 0
            if NetTotal > 0 then begin
            ...
            end else if (NetTotal < 0) then begin
